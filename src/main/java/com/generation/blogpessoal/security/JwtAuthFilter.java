	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		try {
			if(authHeader != null && authHeader.startsWith("Bearer ")){
				...
				username = jwtService.extractUsername(token); //chama a jwtService para extrair o usuário que esta no token
			}
			
			//validação de existe um user name que foi extraido do token e não temos regras configuradas de autorização
			...
			//aqui vamos ter o status setado para caso o usuário não for validado por algum motivo
